...
def mock_api_client():
    ...
    client.create.return_value = {"region_name": "res-123", "region_name": "test-instance_availability_info"}
    client.update.return_value = {"region_name": "res-123", "region_name": "test-instance_availability_info-updated"}
...
class TestCreateInstanceAvailabilityInfo:
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        params = {
            "region_name": "full-instance_availability_info",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("instance_availability_info", params)
        mock_api_client.create.assert_called_once_with("instance_availability_info", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("instance_availability_info", {"region_name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "instance_availability_info": {}}
        else:
...
class TestUpdateInstanceAvailabilityInfo:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("instance_availability_info", "res-123", {"region_name": "updated"})
        assert result["region_name"] == "test-instance_availability_info-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"region_name": existing_resource["region_name"]}
        current = {"region_name": existing_resource["region_name"]}
...
class TestDeleteInstanceAvailabilityInfo:
    ...
    def test_delete_existing(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("instance_availability_info", "res-123")
        mock_api_client.delete.assert_called_once_with("instance_availability_info", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("instance_availability_info", "missing-id")
...
class TestGetInstanceAvailabilityInfo:
    ...
    def test_get_existing(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        result = mock_api_client.get("instance_availability_info", "res-123")
        assert result["region_name"] == "res-123"

    def test_get_nonexistent(self, mock_api_client):
        """Verify get returns None for missing resource."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("instance_availability_info", "nonexistent")
        assert result is None
...
class TestListInstanceAvailabilityInfo:
    ...
    def test_list_returns_all(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [
            {"region_name": "1", "region_name": "first"},
            {"region_name": "2", "region_name": "second"},
        ]
...
class TestIdempotencyInstanceAvailabilityInfo:
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("instance_availability_info", "res-123")
        desired_params = {"region_name": current["region_name"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["region_name"] != current["region_name"]
        assert changed is False
...
class TestErrorHandlingInstanceAvailabilityInfo:
    ...
    def test_auth_failure(self):
        ...
        with pytest.raises(Exception, match="401 Unauthorized"):
            client.create("instance_availability_info", {})
