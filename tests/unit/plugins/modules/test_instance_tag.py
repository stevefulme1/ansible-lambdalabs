...
def mock_api_client():
    ...
    client.create.return_value = {"instance_id": "res-123", "key": "test-instance_tag"}
    client.update.return_value = {"instance_id": "res-123", "key": "test-instance_tag-updated"}
...
class TestCreateInstanceTag:
    ...
    def test_create_returns_resource(self, mock_api_client):
        ...
        result = mock_api_client.create("instance_tag", {"key": "test-instance_tag"})
        assert result["instance_id"] == "res-123"
        assert result["key"] == "test-instance_tag"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "key": "full-instance_tag",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("instance_tag", params)
        mock_api_client.create.assert_called_once_with("instance_tag", params)
...
class TestUpdateInstanceTag:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("instance_tag", "res-123", {"key": "updated"})
        assert result["key"] == "test-instance_tag-updated"
...
class TestDeleteInstanceTag:
    ...
    def test_delete_existing(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("instance_tag", "res-123")
        mock_api_client.delete.assert_called_once_with("instance_tag", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("instance_tag", "missing-id")
...
class TestGetInstanceTag:
    ...
    def test_get_existing(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        result = mock_api_client.get("instance_tag", "res-123")
        assert result["instance_id"] == "res-123"

    def test_get_nonexistent(self, mock_api_client):
        """Verify get returns None for missing resource."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("instance_tag", "nonexistent")
        assert result is None
...
class TestListInstanceTag:
    ...
    def test_list_returns_all(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [
            {"instance_id": "1", "key": "first"},
            {"instance_id": "2", "key": "second"},
        ]
        result = mock_api_client.list("instance_tag")
        assert len(result) == 2
...
class TestIdempotencyInstanceTag:
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("instance_tag", "res-123")
