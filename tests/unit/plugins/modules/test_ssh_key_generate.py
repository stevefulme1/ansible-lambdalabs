"""Unit tests for stevefulme1.lambdalabs.ssh_key_generate module."""
...
def mock_api_client():
    ...
    client.create.return_value = {"key_id": "res-123", "name": "test-ssh_key_generate"}
    client.update.return_value = {"key_id": "res-123", "name": "test-ssh_key_generate-updated"}
...
class TestCreateSshKeyGenerate:
    ...
    def test_create_returns_resource(self, mock_api_client):
        ...
        result = mock_api_client.create("ssh_key_generate", {"name": "test-ssh_key_generate"})
        assert result["key_id"] == "res-123"
        assert result["name"] == "test-ssh_key_generate"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "name": "full-ssh_key_generate",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("ssh_key_generate", params)
        mock_api_client.create.assert_called_once_with("ssh_key_generate", params)
...
class TestUpdateSshKeyGenerate:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("ssh_key_generate", "res-123", {"name": "updated"})
        assert result["name"] == "test-ssh_key_generate-updated"
...
class TestDeleteSshKeyGenerate:
    ...
    def test_delete_existing(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("ssh_key_generate", "res-123")
        mock_api_client.delete.assert_called_once_with("ssh_key_generate", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("ssh_key_generate", "missing-id")
...
class TestGetSshKeyGenerate:
    ...
    def test_get_existing(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        result = mock_api_client.get("ssh_key_generate", "res-123")
        assert result["key_id"] == "res-123"

    def test_get_nonexistent(self, mock_api_client):
        """Verify get returns None for missing resource."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("ssh_key_generate", "nonexistent")
        assert result is None
...
class TestListSshKeyGenerate:
    ...
    def test_list_returns_all(self, mock_api_client):
        ...
        result = mock_api_client.list("ssh_key_generate")
        assert len(result) == 2
...
class TestIdempotencySshKeyGenerate:
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("ssh_key_generate", "res-123")
...
class TestErrorHandlingSshKeyGenerate:
    ...
    def test_auth_failure(self):
        ...
        with pytest.raises(Exception, match="401 Unauthorized"):
            client.create("ssh_key_generate", {})
