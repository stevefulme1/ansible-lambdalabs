"""Unit tests for stevefulme1.lambdalabs.instance module."""

from unittest.mock import MagicMock

import pytest

MODULE_PATH = "ansible_collections.stevefulme1.lambdalabs.plugins.modules.instance"
CLIENT_PATH = "ansible_collections.stevefulme1.lambdalabs.plugins.module_utils.lambda_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for instance."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"instance_id": "res-123", "name": "test-instance"}
    client.update.return_value = {"instance_id": "res-123", "name": "test-instance-updated"}
...
def existing_resource():
    """Return a dict representing an existing instance."""
    return {
        "instance_id": "res-123",
        "name": "test-instance",
        "state": "active",
...
class TestCreateInstance:
    """Tests for creating a instance."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("instance", {"name": "test-instance"})
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("instance", params)
        mock_api_client.create.assert_called_once_with("instance", params)
...
class TestUpdateInstance:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("instance", "res-123", {"name": "updated"})
        assert result["name"] == "test-instance-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("instance", "bad-id", {})


class TestDeleteInstance:
    """Tests for deleting a instance."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("instance", "res-123")
        mock_api_client.delete.assert_called_once_with("instance", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("instance", "missing-id")
...
class TestGetInstance:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("instance", "res-123")


class TestListInstance:
    """Tests for listing instance resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"instance_id": "1", "name": "first"},
            {"instance_id": "2", "name": "second"},
        ]
        result = mock_api_client.list("instance")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"instance_id": "1", "name": "match"}]
        result = mock_api_client.list("instance", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencyInstance:
    """Tests for idempotent behavior of instance."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("instance", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("instance", "missing") is not None
        assert exists is False


class TestErrorHandlingInstance:
    """Tests for error handling in instance."""
