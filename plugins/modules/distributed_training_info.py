...
"""List or get details about Lambda Labs distributed training jobs."""
...
DOCUMENTATION = r"""
---
module: distributed_training_info
short_description: Get Lambda Labs distributed training job info
description:
  - Retrieve information about distributed training jobs on Lambda Labs.
version_added: "1.0.0"
...
EXAMPLES = r"""
- name: List all training jobs
  stevefulme1.lambdalabs.distributed_training_info:
    api_key: "{{ lambda_api_key }}"
  register: result

- name: Get a specific training job
  stevefulme1.lambdalabs.distributed_training_info:
    api_key: "{{ lambda_api_key }}"
    job_id: job-abc123
"""
...
def main():
    ...
    try:
        ...
        if jid:
            ...
            jobs = result.get("data", [])
        module.exit_json(changed=False, training_jobs=jobs)
